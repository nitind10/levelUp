...
// with Path Compression :  alpha(n)  <= 4, Inverse Akermann Function
...
// Path Compression: V + E(alpha(n)) (ours is this one)
// without Path Compression: V + ELog(V)
void unionFind(int N, vector<vector<int>> &Edges)
{
    //this graph created will be a spanning tree
    ...
    for (vector<int> &edge : Edges) //O(E)
    {
        ...
        int p1 = findPar(u); //alpha(n)
        int p2 = findPar(v); //alpha(n)
...
{   
    int N = 4;
    vector<vector<int>> Edges {{0,1,5}, {1,2,2}, {2,3,3}, {3,0,1}};
